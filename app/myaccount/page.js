...
const Dashboard = () => {
  ...
  async function auth() {
    ...
    if (data.success) {
      ...
      getuserOrders()
      if (redirected === 'true') {
        toast.error(`Provide Address, to place Order.`, {
          position: "top-center",
          autoClose: 1000,
  ...
  async function getuserOrders() {
    setLoading(true)
    const fetch_api = await fetch("/api/getorders", {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    });
  ...
  async function getUserDetails() {
    const fetch_api = await fetch("/api/auth?fulldetails=true", {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    });
    ...
    if (data.success) {
      ...
      setContactNumber(data.user_details.phone)
      setFullAddress(data.user_details.address)
      setPincode(data.user_details.pincode)
      setCityState(data.user_details.state)
    }
  }
  ...
  const handleSave = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.log(`${errorMessage.msg} for ${errorMessage.path}`);
        toast.error(`${errorMessage.msg} for ${errorMessage.path}`, {
          position: "top-center",
          autoClose: 1000,
          hideProgressBar: false,
          closeOnClick: true,
  ...
  return (
    <>
      {ValidUser ? <section className="flex justify-center">
        <ToastContainer />
        <div className={`flex container justify-center  md:flex-row ${redirected === 'true' ? 'flex-col-reverse' : 'flex-col'}`}>
            ...
            {Orders.length ? Orders.slice().reverse().map((order, index) => (
              <div key={index} className="p-2 w-full">
                <div className="h-full flex items-center border-gray-200 border cursor-pointer p-4 rounded-lg hover:bg-gray-100">
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fillRule="black" className="bi bi-chevron-right" viewBox="0 0 16 16">
